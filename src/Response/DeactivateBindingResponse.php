...
/**
 * The answer to DeactivateBinding.
 *
 * Model of record: `DeactivateBindingResponse` in
 * docs/api-reference/api-surface.json. Three fields, verbatim
 * (CONVENTIONS.md §4.8). The endpoint name is singular (CONVENTIONS.md §4.9).
 *
 * Structurally identical to ActivateBindingResponse and kept separate for the
 ...
 *
 * Observed only as a refusal: probe A1.3 got `ResponseCode` "20", "Client
 * payment type BindingMainRest is not available" (CONVENTIONS.md §13). A
 * successful body has never been seen.
 *
 * @todo unverified — see CONVENTIONS.md §13 (bindings are not permitted on the sandbox client)
 */
final readonly class DeactivateBindingResponse
{
    /**
    ...
    {
        return ResponseHydrator::deactivateBindingResponse($data);
    }
}
